    ...
    D: Deserializer<'de>,
{
    ...
    // Para PerDcomp, usamos NumberFormat::Brazilian como padrão
    format_number(&s, NumberFormat::Brazilian)
        .parse::<f64>()
        .map_err(|_| Error::custom(format!("Falha ao converter '{}' para f64", s)))
}
